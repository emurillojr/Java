...
Write a program that reads a row of integers and determines which is the largest and how many times it occurred.  When the user enters a 0, the program will output the calculations.  
Note – the input is all on one line, there is no carriage returns between numbers.  The output should look like the sample below.
